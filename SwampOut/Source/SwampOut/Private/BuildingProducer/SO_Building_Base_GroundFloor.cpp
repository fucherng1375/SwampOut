...
#include "BuildingProducer/SO_Building_Base_Level.h"
...
void ASO_Building_Base_GroundFloor::ClassifyBuildingData()
{
	...
	{
		EVariant KeyAccess = data.Level->GetDefaultObject<ASO_Building_Base_Level>()->EntryID;
		...
		{
			SeparateVariant.Add(data.Level->GetDefaultObject<ASO_Building_Base_Level>()->EntryID, FBuildingMultipleLevelData(data));
		}
	}
...
void ASO_Building_Base_GroundFloor::Auth_SpawnBuilding()
{
	...
	TSubclassOf<ASO_Building_Base_GroundFloor> GroundFloor = FindRow->RandomGroundFloor[FMath::RandRange(0 , (FindRow->RandomGroundFloor.Num() - 1))];
	if (!IsValid(GroundFloor)) return;
	CurrentExitID.Add(GroundFloor->GetDefaultObject<ASO_Building_Base_GroundFloor>()->ExitID);
	...
	for (int32 i = 0; i < NumberOfLevel; i++)
	{
		...
		if (!IsValid(GetRandomLevel(LevelData).Level)) continue;

		CurrentExitID.Add(GetRandomLevel(LevelData).Level->GetDefaultObject<ASO_Building_Base_Level>()->ExitID);
		FilteredLevels.Add(GetRandomLevel(LevelData));
		ReadyToSpawns.Add(GetRandomLevel(LevelData).Level);
	}
	...
	for (auto& level : ReadyToSpawns)
	{
		...
		CacheLevel.Add(GetWorld()->SpawnActor<ASO_Building_Base_Level>(level, SpawnTransform));
		OnSpawningLevel(CacheLevel[CacheLevel.Num() - 1]);
	}
}

void ASO_Building_Base_GroundFloor::Auth_SpawnBuildingOld()
{
	/*
	if (!HasAuthority() || !IsValid(DataTable)) return;
	...
	TArray<TSubclassOf<ASO_Building_Base_Level>> HasSpawned;
		...
		for (FBuildingSingleLevelData& SA : SortedArray)
		{
			...
			if (RandomValue <= SA.SpawnWeight && VariantSpawnedCount < SA.SpawnLimit)
			{
				...
				if (SA.Repeat ? true : !HasSpawned.Contains(SA.Level))
				{
					...
					FTransform SpawnTransform;
					CacheLevel.Add(GetWorld()->SpawnActor<ASO_Building_Base_Level>(SA.Level, SpawnTransform));
					OnSpawningLevel(CacheLevel[CacheLevel.Num()-1]);
					break;
				}
				else
